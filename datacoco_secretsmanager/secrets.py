...
class SecretsManager:
    ...
    """
    def __init__(
        self, aws_access_key_id=None, aws_secret_access_key=None, aws_role_arn=None, region_name='us-east-1'
    ):
        """
        :param aws_access_key_id: use to override default credentials or assumed role access
        :param aws_secret_access_key: use to override default credentials or assumed role access
        :param aws_role_arn: for assuming role with secrets must be used if passing in access keys
        ...
        if aws_access_key_id and aws_secret_access_key:
            if aws_role_arn:
                ...
                warnings.warn(
                    "If role based permissions used in AWS, aws_role_arn arg must be provided for access to secrets' resource(s).")
    ...
    def get_secret(self, secret_name):
        ...
        :param secret_name: resource name entered in AWS, with credentials stored as key(s)/value(s)
        """
    ...
    def get_config(self, project_name, team='data'):
        """Fetch full config and credentials for all systems/connections at repo/project level.
        ...
        if config_lookup:
            for k, v in config_lookup.items():
                ...
                section_config = {
                    config_item[0]: config_item[1] for config_item in section.items()
                }
